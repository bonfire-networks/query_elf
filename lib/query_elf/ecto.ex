if Code.ensure_loaded?(Ecto) do
  defmodule QueryElf.Ecto do
    @moduledoc """
    An adapter for the `QueryElf` behaviour that makes it easier to implement a query
    builder around an `Ecto.Query`. Here's a simple usage example:
    ...
    If used, the `:searchable_fields` option will define the following filters (according to the
    ...
    @doc """
    Should return what is the default page size for this particular schema.
    """
    @callback default_per_page :: non_neg_integer
    ...
    @optional_callbacks [sort: 4, default_per_page: 0, base_query: 0]
    ...
    defp apply_pagination(query, builder, page, nil),
      do: apply_pagination(query, builder, page, builder.default_per_page())
